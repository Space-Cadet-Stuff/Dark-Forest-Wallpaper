...
const rainBgCanvas = document.createElement('canvas');
rainBgCanvas.id = 'rain-bg-overlay';
rainBgCanvas.style.position = 'fixed';
rainBgCanvas.style.top = '0';
rainBgCanvas.style.left = '0';
rainBgCanvas.style.width = '100vw';
rainBgCanvas.style.height = '100vh';
rainBgCanvas.style.zIndex = '-0.5';
rainBgCanvas.style.pointerEvents = 'none';
document.body.appendChild(rainBgCanvas);
const rainBgCtx = rainBgCanvas.getContext('2d');

let raindropsBg = [];
const RAIN_BG_DENSITY = 0.012;
const RAIN_BG_SPEED = 11;
const RAIN_BG_LENGTH = 22;
const RAIN_BG_WIDTH = 1.2;
const RAIN_BG_WIND = 5;

function resizeRainBgCanvas() {
	rainBgCanvas.width = window.innerWidth;
	rainBgCanvas.height = window.innerHeight;
}
window.addEventListener('resize', resizeRainBgCanvas);
resizeRainBgCanvas();

function spawnRaindropsBg() {
	const count = Math.floor(window.innerWidth * RAIN_BG_DENSITY);
	const sidePad = 400;
	for (let i = 0; i < count; i++) {
		raindropsBg.push({
			x: -sidePad + Math.random() * (window.innerWidth + sidePad * 2),
			y: Math.random() * -window.innerHeight,
			speed: RAIN_BG_SPEED + Math.random() * 2,
			len: RAIN_BG_LENGTH + Math.random() * 7,
			wind: RAIN_BG_WIND + Math.random() * 1.5,
			width: RAIN_BG_WIDTH + Math.random() * 0.7
		});
	}
}

function updateRainBg() {
	for (let i = raindropsBg.length - 1; i >= 0; i--) {
		let drop = raindropsBg[i];
		drop.x += drop.wind;
		drop.y += drop.speed;
		if (drop.y > window.innerHeight - 18 || drop.x > window.innerWidth + 40) {
			raindropsBg.splice(i, 1);
		}
	}
	if (raindropsBg.length < window.innerWidth * RAIN_BG_DENSITY * 2) {
		spawnRaindropsBg();
	}
}

function drawRainBg() {
	rainBgCtx.clearRect(0, 0, rainBgCanvas.width, rainBgCanvas.height);
	rainBgCtx.save();
	rainBgCtx.strokeStyle = 'rgba(255,255,255,0.25)';
	for (let drop of raindropsBg) {
		rainBgCtx.beginPath();
		rainBgCtx.lineWidth = drop.width;
		rainBgCtx.moveTo(drop.x, drop.y);
		rainBgCtx.lineTo(drop.x + drop.wind * 1.1, drop.y + drop.len);
		rainBgCtx.stroke();
	}
	rainBgCtx.restore();
}

function animateRainBg() {
	updateRainBg();
	drawRainBg();
	requestAnimationFrame(animateRainBg);
}
spawnRaindropsBg();
animateRainBg();
...
const RAIN_DENSITY = 0.09;
const RAIN_SPEED = 19;
const RAIN_LENGTH = 32;
const RAIN_WIDTH = 2.2;
const RAIN_WIND = 8;
const SPLASH_PARTICLES = 10;
const SPLASH_LIFETIME = 14;
...
function spawnRaindrops() {
	const count = Math.floor(window.innerWidth * RAIN_DENSITY);
	const sidePad = 400;
	for (let i = 0; i < count; i++) {
		raindrops.push({
			x: -sidePad + Math.random() * (window.innerWidth + sidePad * 2),
			y: Math.random() * -window.innerHeight,
			speed: RAIN_SPEED + Math.random() * 4,
			len: RAIN_LENGTH + Math.random() * 10,
			wind: RAIN_WIND + Math.random() * 2,
			width: RAIN_WIDTH + Math.random() * 1.2
		});
	}
}
...
function updateRain() {
	...
	for (let i = raindrops.length - 1; i >= 0; i--) {
		...
		drop.x += drop.wind;
		drop.y += drop.speed;
		if (drop.y > groundY || drop.x > window.innerWidth + 40) {
			createSplash(drop.x, groundY, drop.wind);
			raindrops.splice(i, 1);
		}
	}
	if (raindrops.length < window.innerWidth * RAIN_DENSITY * 2) {
		spawnRaindrops();
	}
	for (let i = splashes.length - 1; i >= 0; i--) {
		...
		s.y += s.vy;
		s.vy += 0.4;
		if (s.life > SPLASH_LIFETIME) {
			splashes.splice(i, 1);
		}
	}
}

function createSplash(x, y, wind = RAIN_WIND) {
	for (let i = 0; i < SPLASH_PARTICLES; i++) {
		...
		splashes.push({
			x: x,
			y: y,
			vx: Math.cos(angle) * speed * 0.25 * (Math.random() < 0.5 ? -1 : 1),
...
function drawRain() {
	...
	for (let s of splashes) {
		let alpha = 1 - s.life / SPLASH_LIFETIME;
		rainCtx.save();
		rainCtx.globalAlpha = alpha * 0.8;
		rainCtx.beginPath();
		let arcStart = Math.PI;
		let arcEnd = 2 * Math.PI;
		rainCtx.arc(s.x, s.y, 2.2 + 1.7 * (1 - alpha), arcStart, arcEnd, true);
		rainCtx.strokeStyle = '#fff';
